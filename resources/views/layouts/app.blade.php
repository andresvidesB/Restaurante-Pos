    ...
    <div x-data="{ sidebarOpen: false }" class="flex h-screen overflow-hidden bg-gray-100">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
            ...
            <nav class="flex-1 px-2 py-4 space-y-2 overflow-y-auto">
                ...
                <a href="{{ route('ofertas.index') }}" class="flex items-center px-4 py-2 rounded transition-colors {{ request()->routeIs('ofertas.index') ? 'bg-slate-900 text-yellow-400 shadow-inner' : 'text-gray-300 hover:bg-slate-700 hover:text-white' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 18.657A8 8 0 016.343 7.343S7 9 9 10c0-2 .5-5 2.986-7C14 5 16.09 5.777 17.656 7.343A7.975 7.975 0 0120 13a7.975 7.975 0 01-2.343 5.657z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.879 16.121A3 3 0 1012.015 11L11 14H9c0 .768.293 1.536.879 2.121z"></path></svg>
                    Gestión de Ofertas
                </a>
                @if(auth()->user()->role === 'admin')
    <a href="{{ route('usuarios.index') }}" class="flex items-center px-4 py-2 rounded transition-colors {{ request()->routeIs('usuarios.index') ? 'bg-slate-900 text-yellow-400 shadow-inner' : 'text-gray-300 hover:bg-slate-700 hover:text-white' }}">
        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.35l-7.1 14.95 1.4 1.4L12 17.5l5.7 3.2 1.4-1.4-7.1-14.95zM12 14v-4m0-4h.01"></path>
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 21v-2a4 4 0 00-4-4H9a4 4 0 00-4 4v2"></path>
        </svg>
        Gestión de Usuarios
    </a>
@endif
